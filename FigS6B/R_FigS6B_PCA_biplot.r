...
#---Initializing:  setting all names, sizes, colors, read in data etc.: #####			
	...
	model_name <- "05_hELISAL_Donor_COPD_conc_tprotNorm_OEME_LOG_0quarterofmin_replaced_cohort2"
	...
	output_folder <- paste0(statisticsfilepath,"output_FigS6B/")
	...
	Biplot_cPCA_PC23 <- fviz_pca_biplot(PCA_modelobject,  axes = c(2, 3), title = plot_title, 
    ...
    xlab(xlabel_PC2) + ylab(ylabel_PC3) + theme_journal +   # +coord_fixed() #makes quadratic
  ...
  ggsave(paste0(output_folder, model_name,"__cPCA_PC23_biplot.png"),plot = Biplot_cPCA_PC23, device = png(), width=w+5, 
         height=h+7, units = "mm", dpi = 900); dev.off()
  ggsave(paste0(output_folder, model_name,"__cPCA_PC23_biplot.pdf"),plot = Biplot_cPCA_PC23, device = pdf(), width=w+5, 
         height=h+7, units = "mm", dpi = 900); dev.off()
