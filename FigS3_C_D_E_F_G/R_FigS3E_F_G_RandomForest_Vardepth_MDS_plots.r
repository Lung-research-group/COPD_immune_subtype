...
#---Initializing: read in and prepare data: #####			
  ...
  #Set path where to find all .R files and folders with input/output
	...
	model_name <- "04_hFACS_Donor_COPD_Lung_C_cc_LOG_x1c_filtDD"
	...
	output_folder <- paste0(statisticsfilepath,"output_FigS3E_F_G/")
	...
	ggsave(paste0(output_folder, "FigS3F__", model_name,"__MDS_Scores.png") ,plot = RFPlot_MDS, 
	       device = png(), width=w, height=h, units = "mm", dpi = 600); dev.off()
	ggsave(paste0(output_folder, "FigS3F__", model_name,"__MDS_Scores.pdf") ,plot = RFPlot_MDS, 
	...
	ggsave(paste0(output_folder, "FigS3E__", model_name,"__VarImpminDepth.png"), 
	       plot = RFplot_min_depth_all_png, device = png(), width=w_vimd, height=h_vimd, units = "mm", dpi = 600)
	dev.off()
	ggsave(paste0(output_folder, "FigS3E__", model_name,"__VarImpminDepth.pdf"), 
	...
	ggsave(paste0(output_folder, "FigS3E_right_", model_name,"__groupLFC_map.png"), 
	       plot = RFplot_groupLFC_map, device = png(), width=w_vimd-34+4*(n_lvls-1), height=h_vimd+4, units = "mm", dpi = 600)
	dev.off() 
	ggsave(paste0(output_folder, "FigS3E_right_",model_name,"__groupLFC_map.pdf"), 
	...
	ParamID_top300T <- c("CD3", "CD4", "CD8", "CD19", "Neutrophils", "Mono_int")
...
#---RF model MDS export results   ########--------#####	     
  ...
  ggsave(paste0(output_folder, "FigS3G__", model_name,"__MDS_Scores_top300T.png") ,plot = RFPlot_MDS_top300T, 
         device = png(), width=w, height=h, units = "mm", dpi = 600); dev.off()
  ggsave(paste0(output_folder, "FigS3G__", model_name,"__MDS_Scores_top300T.pdf") ,plot = RFPlot_MDS_top300T, 
         device = pdf(), width=w, height=h, units = "mm", dpi = 600); dev.off()
